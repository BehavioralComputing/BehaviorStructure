...
with open('7d_column_encoding.json', "r") as file:
...
for i in range(10):
    ...
    for epoch in range(300):
        ...
        if acc > best_val_acc:
            best_val_acc = acc
            torch.save(model.state_dict(), '7d_RGCN_MLP_best_model.pt')
    ...
    model.load_state_dict(torch.load('7d_RGCN_MLP_best_model.pt'))
    model.eval()
    test_predictions = []
    test_labels = []
    ...
    with torch.no_grad():
        ...
        all_reports = f"7d_RCGN_Classification Report hid_num {hid_num} round {i} layers_2:\n{report}\n\n"
        with open("./report/7d_RGCN_MLP_classification_reports.txt", "a") as report_file:
