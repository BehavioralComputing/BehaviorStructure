...
with open(prefix+'column_encoding.json', "r") as file:
...
for i in range(10):
    ...
    train_idx, val_idx = train_test_split(train_idx, test_size=0.5, random_state=i)
    ...
    for epoch in range(300):
        ...
        if acc > best_val_acc:
            best_val_acc = acc
            torch.save(model.state_dict(), prefix+'RGCN_MLP_best_model.pt')


        print(
        "Epoch {:05d} | Loss {:.4f} | Accuracy {:.4f} | Time {:.4f}s".format(
            epoch, loss.item(), acc, time.time() - start_time
                )
            )
        
    model.load_state_dict(torch.load(prefix+'RGCN_MLP_best_model.pt'))
    model.eval()
    test_predictions = []
    test_labels = []
    ...
    with torch.no_grad():
        ...
        all_reports = f'{prefix}RCGN_Classification Report hid_num {hid_num} round {i} layers_2:\n{report}\n\n'
        with open('./report/'+prefix+'RGCN_MLP_classification_reports.txt', "a") as report_file:
